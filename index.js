...
StringChallenge (num) take the num parameter being passed and return the number of hours and minutes the parameter converts to 
...
Final Output: 2q:w6f9yt2x013

Input: 45

Output: 0:45

Final Output: 0q:w4f59yt2x013
...
//O/P = { pending: 3, complete: 1 }

let result1 = testArray.filter((val) => val.status == "pending").length;

let result2 = testArray.filter((val) => val.status == "complete").length;

/* finalResult = {pending:result1,completed:result2}
...
for (let i = 0; i < testArray.length; i++) {
  if (finalResult[testArray[i].status]) {
    finalResult[testArray[i].status]++;
  } else {
    finalResult[testArray[i].status] = 1;
  }
}
